...
class ControleVendasApp:
    ...
    def criar_widgets(self):
        ...
        tk.Button(frame_produtos, text="Adicionar Produto", command=self.adicionar_produto).pack(fill="x", pady=2)
        tk.Button(frame_produtos, text="Remover Produto", command=self.remover_produto).pack(fill="x", pady=2)
        ...
        tk.Button(frame_motoboys, text="Adicionar Motoboy", command=self.adicionar_motoboy).pack(fill="x", pady=2)
        tk.Button(frame_motoboys, text="Remover Motoboy", command=self.remover_motoboy).pack(fill="x", pady=2)
        ...
        tk.Button(frame_pedidos, text="Adicionar Pedido", command=self.adicionar_pedido).pack(fill="x", pady=2)

        # Exportação
        frame_exportacao = tk.Frame(self.tab_exportacao)
        frame_exportacao.pack(fill="both", expand=True)
        tk.Button(frame_exportacao, text="Exportar para Excel", command=self.exportar_excel).pack(fill="x", pady=5)
        ...
        tk.Button(frame_relatorios, text="Gerar por Período", command=self.relatorio_por_periodo).pack(fill="x", pady=2)
        tk.Button(frame_relatorios, text="Gerar por Motoboy", command=self.relatorio_por_motoboy).pack(fill="x", pady=2)
        tk.Button(frame_relatorios, text="Gerar por Tipo de Pedido", command=self.relatorio_por_tipo).pack(fill="x", pady=2)
    ...
    def adicionar_motoboy(self):
        ...
        if nome:
            if nome in self.motoboys:
                messagebox.showwarning("Aviso", "Motoboy já cadastrado.")
                return
    ...
    def adicionar_pedido(self):
        ...
        if not tipo:
            messagebox.showerror("Erro", "Tipo de pedido obrigatório.")
            return
    ...
    def relatorio_por_periodo(self):
        ...
        try:
            ...
            df_filtrado = df[(df['Data'] >= pd.to_datetime(inicio)) & (df['Data'] <= pd.to_datetime(fim))]
            if df_filtrado.empty:
                messagebox.showinfo("Info", "Nenhum pedido encontrado nesse período.")
                return
