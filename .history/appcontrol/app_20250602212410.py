...
class App(tk.Tk):
    ...
    def _configurar_estilo(self):
        ...
        # Configurações específicas para campos de entrada
        style.configure('Dark.TEntry', 
                    fieldbackground='#2d2d2d', 
                    foreground=DARK_FG,
                    insertcolor=DARK_FG)  # Cor do cursor
    ...
    def criar_arquivos_iniciais():
        dados_iniciais = {
            'produtos.json': [
                {"nome": "Frango Assado", "quantidade": 10, "preco": 25.90},
                {"nome": "Coca-Cola Lata", "quantidade": 50, "preco": 5.00}
            ],
            'motoboys.json': [
                {"nome": "João", "valor_por_entrega": 8.00},
                {"nome": "Carlos", "valor_por_entrega": 7.50}
            ],
            'pedidos.json': []
        }
    
    for arquivo, dados in dados_iniciais.items():
        if not os.path.exists(arquivo):
            with open(arquivo, 'w', encoding='utf-8') as f:
    ...
    def _criar_aba_pedidos(self):
        ...
        estilo = ttk.Style()
        estilo.configure('Dark.TEntry', fieldbackground='#2d2d2d', foreground=DARK_FG)
        estilo.configure('Dark.TCombobox', fieldbackground='#2d2d2d', foreground=DARK_FG)
        ...
        self.combo_motoboy = ttk.Combobox(self.motoboy_frame, state="readonly", 
                                        width=27, style='Dark.TCombobox')
    ...
    def _editar_pedido(self):
        ...
        if pedido:
            ...
            self.entry_cliente.insert(0, pedido.get('cliente', ''))
    ...
    def _excluir_pedido(self):
        ...
        item = self.tree_pedidos.item(selecionado)
        pedido_id = item['values'][0]
        produto_nome = item['values'][4]
        quantidade = int(item['values'][5])
    ...
    def _adicionar_pedido(self):
        ...
        if not all([cliente, produto, qtd_str]):
